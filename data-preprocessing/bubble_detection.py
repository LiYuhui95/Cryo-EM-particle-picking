...
#blobs_log[:, 2] = blobs_log[:, 2] * sqrt(2)
    ...
    result[np.where(dilated<np.mean(dilated)*0.94)]=1
    
    img = img_as_float(img)
    img_normal_mean = np.mean(img[np.where(dilated>=np.mean(dilated)*0.94)])
    return_image = np.copy(img)
    return_image[np.where(dilated<np.mean(dilated)*0.94)] = img_normal_mean #/ np.mean(img[np.where(dilated<np.mean(dilated)*0.95)])
